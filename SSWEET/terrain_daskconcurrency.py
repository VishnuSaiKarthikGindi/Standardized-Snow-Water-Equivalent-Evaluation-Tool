...
def process_data_in_chunks_1(df, tiles):
    chunk_results = []
    
    futures = [process_single_location((lat, lon, index_id, tiles)) for lat, lon, index_id in tqdm(zip(df['lat'], df['lon'], df['index_id']))]
...
def extract_terrain_data(geospatial_df, num_workers=16):
    ...
    #result_df = pd.concat([geospatial_df, final_result], axis=1)
